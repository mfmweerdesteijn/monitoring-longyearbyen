...
@st.cache_data(ttl=604800, show_spinner='Please be patient while we fetch historic data')
def load_historic_borehole_data_Tilsig(source_string, row_idx):

    # Load historic data from AWS
    source_sn = source_string[:4]
    filename = 'borehole_Tilsig_'+source_sn+'.csv'

    s3_client = boto3.client("s3")
    bucket = "thawinglyr"

    s3_client.download_file(bucket, filename, filename)
    historic_data_df = pd.read_csv(filename)

    # Ensure referenceTime column is of type datetime
    historic_data_df['referenceTime'] = pd.to_datetime(historic_data_df['referenceTime'], format='ISO8601')

    # Load recent data via Tilsig API and process
    recent_data_df = load_recent_boreholes_data_Tilsig(source_string, row_idx)

    # Add recent to historic data
    new_historic_data_df = pd.concat([historic_data_df, recent_data_df], axis=0, ignore_index=True)

    # Upload new historic data to AWS
    s3_resource = boto3.resource('s3')

    csv_buffer = StringIO()
    new_historic_data_df.to_csv(csv_buffer, index=False)

    s3_resource.Object(bucket, filename).put(Body=csv_buffer.getvalue())

    return new_historic_data_df


def load_recent_boreholes_data_Tilsig(source_string, row_idx):

    # Load recent data via Tilsig API
    recent_data_raw = Tilsig_load_recent_borehole_data(source_string)
    # what if there is no data sent over the past week? then this algorithm false apart. need to build in more checks.

    # Convert data to standardized dataframe
    recent_data_df_pre = Tilsig_data_to_dataframe2(recent_data_raw, row_idx)

    # Detect missing depths and timestamps
    recent_data_df_post = data_processing_for_plotting2(recent_data_df_pre)
    
    return recent_data_df_post

def Tilsig_load_recent_borehole_data(source_string):
    ...
    # Send the token request
    r = requests.post(endpoint, json=account_data, headers=headers)
    ...
    if r.status_code == 200:
        ...
        print('Error! Returned status code %s' % r.status_code)
        # here it should do something else than just give an error. stop code or something. and st.text that the authentication to the Tilsig database failed.
    ...
    start_datetime = (datetime.datetime.now() - timedelta(days=7)).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
    end_datetime = (datetime.datetime.now()).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
...
def Tilsig_data_to_dataframe2(data, row_idx):
    # Load into to DataFrame
    df = pd.DataFrame(data)
...
def data_processing_for_plotting2(df):
    # Process data for plotting purposes
    
    ##########################################################
    ### Detect missing depths per timestamp and insert NaN ### 
    ##########################################################
    ...
    df[['stationId', 'deviceId', 'sensorId']] = df[['stationId', 'deviceId', 'sensorId']].astype('int64') 
